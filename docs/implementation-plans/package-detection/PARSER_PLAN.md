...
| #99       | Go workspaces (`go.work`)      | Adds workspace-level ownership and dependency context to an ecosystem that already has strong module and graph support.                           | High leverage because Go parsing and assembly already exist; upstream PR `#4793` is open.                   |
...
These are useful expansions with credible ecosystem reach, but either have narrower usage, weaker assembly leverage, more format variance than the top tier, or weaker current upstream momentum.

| Issue Set      | Opportunity                                                                               | Why this creates value                                                                                                                      | Current overlap / notes                                                                                                                                                                                             |
| -------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| #333           | Bun lockfiles (`bun.lock`, `bun.lockb`)                                                   | Adds resolved dependency evidence for a mainstream JS/TS package manager while reusing the existing npm-family manifest and assembly model. | Best treated as an npm-family lockfile extension, not a separate manifest ecosystem; `package.json` is already covered, `bun.lockb` is legacy, and no open upstream ref-impl issue or PR was visible on 2026-03-14. |
| #113           | Arch Linux (`.SRCINFO`, `.PKGINFO`, `.AURINFO`)                                           | Adds a real distro package family with explicit metadata files and closes part of the earlier `alpm` gap.                                   | More concrete than the older generic `alpm` placeholder; open upstream PR `#4603` covers `.SRCINFO`.                                                                                                                |
| #111, #112     | Installed Python distribution metadata (`WHEEL`, pip cache `origin.json`)                 | Improves installed-package identity, wheel-specific PURL reconstruction, and package-cache scans without inventing a new ecosystem.         | Two open upstream PRs exist: `#4776` for `WHEEL` and `#4708` for pip cache `origin.json`.                                                                                                                           |
| #62            | Yocto / BitBake (`*.bb`)                                                                  | Important for embedded Linux and distro build systems where package metadata lives in recipe files.                                         | Already anticipated in the old purl-gap section; open upstream PR `#3092` exists, but the format still spans high-variance recipe semantics.                                                                        |
| #82, #83       | Nix / Guix package metadata                                                               | Covers important reproducible-build and Linux packaging ecosystems that show up in source distributions and infrastructure repos.           | Valuable ecosystem addition, but parsing semantics are broader and less standardized than lockfile-style formats; no open upstream PR was visible.                                                                  |
...
| #65            | `symfony.lock`                                                                            | Adds framework-specific dependency state for a widely used PHP stack without requiring a whole new ecosystem parser family.                 | Best treated as Composer-adjacent enrichment; upstream PR `#4651` is open.                                                                                                                                          |
...
| #96            | `buildpack.toml`                                                                          | Adds structured metadata for buildpack ecosystems and platform packaging pipelines.                                                         | Clear format, but narrower than mainstream application package managers; upstream PR `#4031` is open.                                                                                                               |
...
| #71            | OpenWrt packages                                                                          | Adds package metadata for embedded/router firmware ecosystems.                                                                              | Niche but meaningful in firmware and distro-style scans; upstream PR `#2400` is still open.                                                                                                                         |
