...
5. **An optional local warm cache is supported, but it is not part of the default distribution contract.**
   The embedded-path startup flow may use an opt-in local `license-index/` cache under the shared
   cache root, but that cache remains an acceleration layer on top of the compact embedded snapshot,
   not a second required distribution mode.

6. **The warm cache is embedded-path only.**
   `--license-rules-path` continues to load directly from the supplied directory and bypasses the
   local license-index cache.
...
- This ADR keeps persistent caching opt-in and does not make the warm cache part of the default
  startup contract.
...
3. **Optional faster repeat startup remains available**
   - Users who opt in can reuse a local `LicenseIndex` snapshot instead of rebuilding it on every
     embedded-path startup.
   - The cache shares the general cache root while remaining separate from scan-result caching.

4. **Simpler repository workflow**
...
1. **Startup is slower than the full prebuilt artifact path**
   ...
   - The warm cache only removes that rebuild cost for users who opt in and stay on the embedded path.

2. **Tracked-path coupling remains broad**
   - Runtime, tests, CI, docs, and release automation all depend on the canonical repo path.

3. **Cache coordination complexity still exists**
   - The optional warm cache adds invalidation and compatibility rules on top of the embedded artifact.
   - Custom rules-directory loading intentionally stays outside that cache path.

4. **Artifact maintenance still exists**
...
- [`src/main.rs`](../../src/main.rs) - startup wiring for embedded-path cache selection
- [`src/cache/config.rs`](../../src/cache/config.rs) - shared cache-root and cache-kind configuration
- [`src/cache/license_index_cache.rs`](../../src/cache/license_index_cache.rs) - opt-in local warm cache for the embedded path
