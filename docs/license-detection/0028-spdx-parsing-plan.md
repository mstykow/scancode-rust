...
2. **Recovery parsing** - Implemented `reparse_invalid_expression()` that:
   ...
   - Added `is_spdx_exception()` to handle exception-only expressions
...
- ✓ Expressions with keywords but invalid structure append `unknown-spdx`
...
**Test Performance Note:** The recovery parsing tests in `test.rs` take ~60s each because they load the full license index from `reference/scancode-toolkit/src/licensedcode/data/licenses`. Consider adding faster unit tests for helper functions that don't require the full index.
...
Per `docs/TESTING_STRATEGY.md`, this implementation follows the multi-layered approach:

### Layer 1: Unit Tests ✓
- **Location:** `src/license_detection/spdx_lid/test.rs`
- **Tests implemented:**
  - `test_recovery_parsing_bare_list` - U-Boot style bare lists
  - `test_recovery_parsing_malformed_parens` - Unbalanced parentheses
  - `test_recovery_parsing_unknown_identifier` - Unknown SPDX keys
  - `test_recovery_parsing_keywords_with_invalid` - Invalid expressions with keywords
  - `test_recovery_parsing_text_after_identifier` - Non-license text filtering
  - `test_uboot_bare_list_as_or` - Integration test with full index
  - `test_is_bare_license_list` - Helper function test

### Layer 2: Golden Tests (Pending Verification)
- **Location:** `src/license_detection/golden_test.rs`
- **Test data:** `testdata/license-golden/datadriven/external/spdx/`
- **Verification needed:** Run golden tests to confirm which SPDX-related failures are fixed

### Recommendations for Improvement:
1. **Add helper function tests** (fast, no index required):
   ```rust
   #[test]
   fn test_tokenize_for_recovery() { ... }
   
   #[test]
   fn test_classify_recovery_token() { ... }
   
   #[test]
   fn test_is_likely_license_key() { ... }
   
   #[test]
   fn test_is_spdx_exception() { ... }
   ```

2. **Run golden tests** to verify actual impact:
   ```bash
   cargo test --release -q --lib license_detection::golden_test -- spdx
   ```

3. **Document golden test baseline** - Record how many tests fail before/after
...
- [x] `reparse_invalid_expression()` handles all test cases
- [x] `convert_recovered_expression_to_scancode()` substitutes unknown keys correctly
- [x] `find_matching_rule_for_expression()` calls recovery parsing at the right point
- [x] U-Boot style bare lists produce OR expressions
- [x] Malformed expressions with keywords append `unknown-spdx`
- [x] Text after identifiers is ignored
- [ ] Golden tests for `uboot.c` pass (not verified)
- [ ] Golden tests for `missing_leading_trailing_paren.txt` pass (not verified)
- [ ] No regressions in valid SPDX expression parsing (not verified)
- [ ] Full test suite passes (not verified)

## Verification History

**2026-03-03 - Verification by Subagent**
- ✓ Recovery parsing functions exist and are implemented correctly
- ✓ All 5 recovery parsing tests pass:
  - `test_recovery_parsing_bare_list`
  - `test_recovery_parsing_malformed_parens`
  - `test_recovery_parsing_unknown_identifier`
  - `test_recovery_parsing_keywords_with_invalid`
  - `test_recovery_parsing_text_after_identifier`
- ✓ U-Boot bare list test passes
- ✓ Implementation matches Python reference behavior in `match_spdx_lid.py:271-340`
- ✓ Rust adds `is_spdx_exception()` which handles edge case not in Python

**Python vs Rust Differences (All Acceptable):**
| Feature | Python | Rust | Notes |
|---------|--------|------|-------|
| Token type filtering | `isinstance(r.value, (LicenseSymbol, Keyword))` | `classify_recovery_token()` pattern match | Equivalent |
| Exception-only handling | Not explicit | `is_spdx_exception()` check | Rust improvement |
| Keyword detection | `has_keywords = True` continue | Same pattern | Equivalent |
| OR for bare lists | `joined_as = ' OR '` if no keywords | Same | Equivalent |
| unknown-spdx witness | `licensing.AND(expression, unknown_symbol)` | Same | Equivalent |

**Testing Strategy Compliance (per docs/TESTING_STRATEGY.md):**
- ✓ Unit tests exist for recovery parsing (Layer 1)
- ✓ Tests cover edge cases (malformed parens, unknown identifiers, text after identifiers)
- ✓ Tests use real SPDX license data from reference
- ⚠ Missing: Unit tests for individual helper functions (`tokenize_for_recovery`, `classify_recovery_token`, `is_likely_license_key`, `is_spdx_exception`)
- ⚠ Missing: Golden test validation section showing specific golden tests affected

**Recommendations:**
1. Add unit tests for helper functions to improve failure isolation
2. Document which golden tests this implementation should fix
3. Consider running golden tests to verify actual impact
