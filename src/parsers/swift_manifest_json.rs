...
/// Parses Swift Package Manager manifest files (Package.swift.json).
///
/// Extracts dependency information from JSON output generated by
/// `swift package dump-package` command. Includes BLAKE3-based caching
/// for significant performance improvements on repeated parses.
///
/// # Performance
/// - **First parse**: ~100-500ms (requires executing `swift package dump-package`)
/// - **Cached parse**: <1ms (BLAKE3 content-based cache hit)
///
/// # Features
/// - Supports all requirement types: exact version, range, branch, revision
/// - Extracts namespace from repository URLs
/// - Graceful fallback when Swift toolchain unavailable
/// - Content-based caching using BLAKE3 hashing
///
/// # Example
/// ```no_run
...
/// use std::path::Path;
///
/// let path = Path::new("Package.swift.json");
/// let package_data = SwiftManifestJsonParser::extract_package_data(path);
/// ```
...
///
/// First checks if a cached version exists based on Package.swift content hash.
/// If cache miss, invokes `invoke_swift_dump_package()` and caches the result.
///
/// # Performance
/// - **Cache hit**: <1ms (read from cache file)
/// - **Cache miss**: ~100-500ms (execute swift command + cache write)
///
/// # Cache Location
/// Cache files stored in system temp directory with BLAKE3 hash as filename.
///
/// # Arguments
/// * `package_swift_path` - Path to Package.swift file
///
/// # Returns
/// * `Ok(String)` - JSON string (from cache or fresh generation)
