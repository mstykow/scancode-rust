...
fn main() {
    ...
    if !skip_seq_matching {
        ...
        if !candidates.is_empty() {
            ...
            let u3_matches: Vec<_> = matches.iter()
        ...
        println!("\nTotal seq_all_matches before merge: {}", seq_all_matches.len());
        
        // Find unicode_3 matches
        let u3_matches: Vec<_> = seq_all_matches.iter()
            .filter(|m| m.rule_identifier == "unicode_3.RULE")
            .collect();
        ...
        let u3_merged: Vec<_> = merged_seq.iter()
            .filter(|m| m.rule_identifier == "unicode_3.RULE")
            .collect();
        println!("unicode_3.RULE after merge: {}", u3_merged.len());
        for m in u3_merged.iter().take(5) {
            println!("  qstart={}, end_token={}, hilen={}, matched_len={}", 
                m.qstart(), m.end_token, m.hilen, m.matched_length);
        }
        ...
        for (i, m) in sorted.iter().enumerate() {
            if m.rule_identifier.contains("unicode_3") || m.rule_identifier.contains("unicode_40") || m.rule_identifier.contains("unicode_42") {
                println!("[{}] {} (qstart={}, end_token={}, hilen={}, len={}, order={})", 
