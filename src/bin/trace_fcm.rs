...
fn main() {
    ...
    println!(
        "u42_aho.qspan_positions: {:?}",
        u42_aho.qspan_positions.as_ref().map(|p| p.len())
    );
    println!(
        "u42_seq.qspan_positions: {:?}",
        u42_seq.qspan_positions.as_ref().map(|p| p.len())
    );
    ...
    if let (Some(u3_pos), Some(u42_pos)) = (&u3.qspan_positions, &u42_seq.qspan_positions) {
        ...
        let intersection: std::collections::HashSet<_> =
            u3_set.intersection(&u42_set).copied().collect();
