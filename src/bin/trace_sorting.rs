...
fn main() {
    ...
    let unicode_3 = all_matches.iter().find(|m| m.rule_identifier == "unicode_3.RULE" && m.start_token == 985);
    let unicode_40 = all_matches.iter().find(|m| m.rule_identifier == "unicode_40.RULE");
    let unicode_42 = all_matches.iter().find(|m| m.rule_identifier == "unicode_42.RULE" && m.matcher == "2-aho");
    ...
    if let Some(m) = unicode_3 {
        ...
        println!("  Sort tuple: (qstart={}, -hilen={}, -len={}, order={})", m.qstart(), -(m.hilen as i64), -(m.matched_length as i64), m.matcher_order());
    }
    ...
    if let (Some(u3), Some(u40), Some(u42)) = (unicode_3, unicode_40, unicode_42) {
        ...
        println!("unicode_3 qspan: start={}, end={}, len={}", u3_qspan.first().unwrap_or(&0), u3_qspan.last().unwrap_or(&0), u3_qspan.len());
        println!("unicode_40 qspan: start={}, end={}, len={}", u40_qspan.first().unwrap_or(&0), u40_qspan.last().unwrap_or(&0), u40_qspan.len());
        println!("unicode_42 qspan: start={}, end={}, len={}", u42_qspan.first().unwrap_or(&0), u42_qspan.last().unwrap_or(&0), u42_qspan.len());
    }
    ...
    for (i, m) in sorted.iter().enumerate() {
        if m.rule_identifier == "unicode_3.RULE" || m.rule_identifier == "unicode_40.RULE" || m.rule_identifier == "unicode_42.RULE" {
            println!("[{}] {} (qstart={}, hilen={}, len={}, order={})", 
                i, m.rule_identifier, m.qstart(), m.hilen, m.matched_length, m.matcher_order());
        }
    }
}
