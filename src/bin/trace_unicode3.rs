...
fn main() {
    ...
    if !skip_seq_matching {
        ...
        if !near_dupe_candidates.is_empty() {
            ...
            let unicode_3_matches: Vec<_> = near_dupe_matches.iter()
            ...
            let unicode_3_merged: Vec<_> = merged_seq.iter()
                .filter(|m| m.rule_identifier == "unicode_3.RULE")
                .collect();
            ...
            let u3_985 = merged_seq.iter().find(|m| m.rule_identifier == "unicode_3.RULE" && m.start_token == 985);
            if let Some(m) = u3_985 {
                println!("\nunicode_3 (qstart=985) EXISTS after merge:");
                println!("  end_token={}, hilen={}, matched_len={}", m.end_token, m.hilen, m.matched_length);
            } else {
                ...
                for m in merged_seq.iter().filter(|m| m.qstart() == 985) {
                    println!("  {} (end_token={}, hilen={}, matched_len={})", 
