...
fn my_filter_contained_matches_verbose(
            ...
            if current.qspan_eq(&next) {
                ...
                if current.match_coverage >= next.match_coverage {
                    if should_print {
                        println!(
                            "  -> DISCARD next {} (cov {} < {})",
                            next.rule_identifier, next.match_coverage, current.match_coverage
                        );
                    }
                    ...
                    if should_print {
                        println!(
                            "  -> DISCARD current {} (cov {} < {})",
                            current.rule_identifier, current.match_coverage, next.match_coverage
                        );
                    }
