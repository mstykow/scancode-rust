...
struct PreparedLineCache<'a> {
    raw_lines: &'a [&'a str],
    prepared: Vec<Option<String>>,
}

impl<'a> PreparedLineCache<'a> {
    fn new(raw_lines: &'a [&'a str]) -> Self {
        Self {
            raw_lines,
    ...
    fn get(&mut self, line_number: usize) -> Option<&str> {
        ...
        if self.prepared[idx].is_none() {
            ...
            self.prepared[idx] = Some(crate::copyright::prepare::prepare_text_line(raw));
        }
...
fn maybe_expand_copyrighted_by_href_urls<'a>(content: &'a str) -> Cow<'a, str> {
...
pub fn detect_copyrights_from_text(
    ...
    if is_lppl_license_document(content) {
        holders.retain(|h| h.holder != "M. Y.");
    }

    drop_arch_floppy_h_bare_1995(content, &mut copyrights);
    drop_batman_adv_contributors_copyright(content, &mut copyrights, &mut holders);

    split_embedded_copyright_detections(&mut copyrights, &mut holders);
    extend_bare_c_year_detections_to_line_end_for_multi_c_lines(
        content,
        &mut copyrights,
        &mut holders,
    );
    replace_holders_with_embedded_c_year_markers(&copyrights, &mut holders);
    add_missing_holders_for_debian_modifications(content, &copyrights, &mut holders);
    fix_sundry_contributors_truncation(content, &mut copyrights, &mut holders);
    restore_bare_holder_angle_emails(&copyrights, &mut holders);
    drop_trailing_software_line_from_holders(content, &mut holders);

    merge_metadata_author_and_email_lines(content, &mut authors);
    extract_debian_maintainer_authors(content, &mut authors);
    extract_maintained_by_authors(content, &mut authors);
    extract_created_by_project_author(content, &mut authors);
    extract_created_by_authors(content, &mut authors);
    extract_written_by_comma_and_copyright_authors(content, &mut authors);
    extract_multiline_written_by_author_blocks(content, &mut authors);
    extract_was_developed_by_author_blocks(content, &mut authors);
    extract_developed_by_sentence_authors(content, &mut authors);
    extract_developed_by_phrase_authors(content, &mut authors);
    extract_with_additional_hacking_by_authors(content, &mut authors);
    extract_developed_and_created_by_authors(content, &mut authors);
    extract_author_colon_blocks(content, &mut authors);
    extract_code_written_by_author_blocks(content, &mut authors);
    extract_converted_to_by_authors(content, &mut authors);
    extract_various_bugfixes_and_enhancements_by_authors(content, &mut authors);
    drop_authors_embedded_in_copyrights(&copyrights, &mut authors);
    drop_shadowed_prefix_authors(&mut authors);
    drop_comedi_ds_status_devices_authors(content, &copyrights, &mut authors);

    merge_implemented_by_lines(content, &mut copyrights, &mut holders, &mut authors);
    ...
    drop_written_by_authors_preceded_by_copyright(content, &mut authors);

    extract_following_authors_holders(content, &mut holders);
    ...
    drop_symbol_year_only_copyrights(content, &mut copyrights);

    drop_from_source_attribution_copyrights(&mut copyrights, &mut holders);

    fix_shm_inline_copyrights(content, &mut copyrights, &mut holders);
    fix_n_tty_linus_torvalds_written_by_clause(content, &mut copyrights, &mut holders);

    merge_freebird_c_inc_urls(content, &mut copyrights, &mut holders);
    merge_debugging390_best_viewed_suffix(content, &mut copyrights, &mut holders);
    merge_fsf_gdb_notice_lines(content, &mut copyrights, &mut holders);
    merge_axis_ethereal_suffix(content, &mut copyrights, &mut holders);
    merge_kirkwood_converted_to(content, &mut copyrights, &mut holders);
    split_reworked_by_suffixes(content, &mut copyrights, &mut holders, &mut authors);
    drop_static_char_string_copyrights(content, &mut copyrights, &mut holders);
    drop_combined_period_holders(&mut holders);
    drop_shadowed_prefix_holders(&mut holders);
    strip_trailing_c_year_suffix_from_comma_and_others(&mut copyrights);
    drop_bare_c_shadowed_by_non_copyright_prefixes(&mut copyrights);
    extract_name_before_rewrited_by_copyrights(content, &mut copyrights, &mut holders);
    extract_developed_at_software_copyrights(content, &mut copyrights, &mut holders);
    extract_confidential_proprietary_copyrights(content, &mut copyrights, &mut holders);
    drop_shadowed_bare_c_holders_with_year_prefixed_copyrights(&mut copyrights, &mut holders);
    drop_shadowed_dashless_holders(&mut holders);
    extract_initials_holders_from_copyrights(&copyrights, &mut holders);
    strip_trailing_the_source_suffixes(&mut copyrights);
    truncate_stichting_mathematisch_centrum_amsterdam_netherlands(&mut copyrights, &mut holders);

    strip_inc_suffix_from_holders_for_today_year_copyrights(&copyrights, &mut holders);

    apply_openoffice_org_report_builder_bin_normalizations(content, &mut copyrights, &mut holders);

    drop_shadowed_bare_c_copyrights_same_span(&mut copyrights);

    drop_copyright_shadowed_by_bare_c_copyrights_same_span(&mut copyrights);
    drop_shadowed_copyright_c_years_only_prefixes(&mut copyrights);

    drop_non_copyright_like_copyrights(&mut copyrights);

    drop_wider_duplicate_holder_spans(&mut holders);

    drop_shadowed_multiline_prefix_copyrights(&mut copyrights);
    drop_shadowed_multiline_prefix_holders(&mut holders);

    drop_shadowed_prefix_copyrights(&mut copyrights);

    drop_shadowed_for_clause_holders_with_email_copyrights(&copyrights, &mut holders);

    drop_shadowed_c_sign_variants(&mut copyrights);
    drop_shadowed_year_prefixed_holders(&mut holders);

    truncate_lonely_svox_baslerstr_address(&mut copyrights, &mut holders);
    add_short_svox_baslerstr_variants(&mut copyrights, &mut holders);
    ...
    add_embedded_copyright_clause_variants(&mut copyrights);
    add_found_at_short_variants(&mut copyrights, &mut holders);
    drop_shadowed_linux_foundation_holder_copyrights_same_line(&mut copyrights);
    add_bare_email_variants_for_escaped_angle_lines(&raw_lines, &mut copyrights);
    drop_comma_holders_shadowed_by_space_version_same_span(&mut holders);
    add_confidential_short_variants_late(&mut copyrights, &mut holders);
    add_karlsruhe_university_short_variants(&mut copyrights, &mut holders);
    add_intel_and_sun_non_portions_variants(&raw_lines, &mut prepared_cache, &mut copyrights);
    add_pipe_read_parenthetical_variants(&raw_lines, &mut prepared_cache, &mut copyrights);
    add_from_url_parenthetical_copyright_variants(&raw_lines, &mut prepared_cache, &mut copyrights);
    add_at_affiliation_short_variants(&mut copyrights, &mut holders);
    add_but_suffix_short_variants(&mut copyrights);
    add_missing_copyrights_for_holder_lines_with_emails(
        &raw_lines,
        &mut prepared_cache,
        &mut copyrights,
        &holders,
    );
    extend_inline_obfuscated_angle_email_suffixes(&mut prepared_cache, &mut copyrights);
    strip_lone_obfuscated_angle_email_user_tokens(&raw_lines, &mut copyrights, &mut holders);
    add_at_domain_variants_for_short_net_angle_emails(&raw_lines, &mut copyrights);
    normalize_french_support_disclaimer_copyrights(&mut copyrights, &mut holders);
    drop_shadowed_email_org_location_suffixes_same_span(&mut copyrights, &mut holders);
    drop_shadowed_plain_email_prefix_copyrights_same_span(&mut copyrights);
    drop_single_line_copyrights_shadowed_by_multiline_same_start(&mut copyrights);
    restore_url_slash_before_closing_paren_from_raw_lines(&raw_lines, &mut copyrights);
    add_first_angle_email_only_variants(&mut copyrights);
    drop_shadowed_angle_email_prefix_copyrights_same_span(&mut copyrights);
    drop_shadowed_quote_before_email_variants_same_span(&mut copyrights);
    add_missing_holder_from_single_copyright(&mut copyrights, &mut holders);

    drop_shadowed_acronym_location_suffix_copyrights_same_span(&mut copyrights);
    drop_copyright_like_holders(&mut holders);
...
fn extract_licensed_material_of_company_bare_c_year_lines(
    ...
    for ln in 1..=prepared_cache.raw_lines.len() {
        let Some(line) = prepared_cache.get(ln) else {
            continue;
        };
        let line = line.trim();
...
fn merge_year_only_copyrights_with_following_author_colon_lines(
    ...
    if prepared_cache.raw_lines.len() < 2 {
        return;
    }
    ...
    for i in 1..prepared_cache.raw_lines.len() {
        let ln1 = i;
        let ln2 = i + 1;
...
fn merge_metadata_author_and_email_lines(content: &str, authors: &mut Vec<AuthorDetection>) {
    let raw_lines: Vec<&str> = content.lines().collect();
...
fn extract_debian_maintainer_authors(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_created_by_project_author(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_created_by_authors(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_written_by_comma_and_copyright_authors(
    content: &str,
    authors: &mut Vec<AuthorDetection>,
) {
    if content.is_empty() {
...
fn extract_multiline_written_by_author_blocks(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_was_developed_by_author_blocks(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_developed_by_sentence_authors(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_developed_by_phrase_authors(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_author_colon_blocks(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_maintained_by_authors(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_code_written_by_author_blocks(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_converted_to_by_authors(content: &str, authors: &mut Vec<AuthorDetection>) {
    if content.is_empty() {
        return;
    }
...
fn extract_various_bugfixes_and_enhancements_by_authors(
    content: &str,
    authors: &mut Vec<AuthorDetection>,
) {
    if content.is_empty() {
...
fn drop_written_by_authors_preceded_by_copyright(
    content: &str,
    authors: &mut Vec<AuthorDetection>,
) {
    if authors.is_empty() {
...
fn drop_authors_embedded_in_copyrights(
    copyrights: &[CopyrightDetection],
    authors: &mut Vec<AuthorDetection>,
) {
...
fn drop_shadowed_prefix_authors(authors: &mut Vec<AuthorDetection>) {
    if authors.len() < 2 {
        return;
    }
...
fn drop_comedi_ds_status_devices_authors(
    content: &str,
    copyrights: &[CopyrightDetection],
    authors: &mut Vec<AuthorDetection>,
) {
...
fn extract_developed_and_created_by_authors(content: &str, authors: &mut Vec<AuthorDetection>) {
...
fn extract_with_additional_hacking_by_authors(content: &str, authors: &mut Vec<AuthorDetection>) {
...
[YEAR] W3C® (MIT, ERCIM, Keio, Beihang).";
        ...
        assert!(
            copyrights.iter().any(|c| {
                c.copyright == "Copyright (c) YEAR W3C(r) (MIT, ERCIM, Keio, Beihang)"
            }),
            "copyrights: {:?}",
            copyrights.iter().map(|c| &c.copyright).collect::<Vec<_>>()
        );
    ...
    fn test_developed_by_sentence_author_is_extracted() {
        let input =
            "developed by the U.S. Government. BAE Systems is enhancing and supporting the SMP";
    ...
    fn test_licco_fixture_merges_author_and_author_email_metadata() {
        ...
        assert!(
            authors.iter().any(|a| {
                a.author == "Hartmut Goebel Author-email h.goebel@crazy-compilers.com"
            }),
            "authors: {:#?}",
            authors.iter().map(|a| &a.author).collect::<Vec<_>>()
        );
    ...
    fn test_libopenraw_fixture_does_not_merge_multiple_debian_copyrights() {
        let path =
            PathBuf::from("testdata/copyright-golden/copyrights/libopenraw1-libopenraw.label");
        let content = fs::read_to_string(&path).expect("read fixture");
        let (copyrights, holders, _authors) = detect_copyrights_from_text(&content);
    ...
    fn test_with_trailing_software_fixture_does_not_append_software_to_holder() {
        let path = PathBuf::from(
            "testdata/copyright-golden/copyrights/copytest/with_trailing_software.txt",
        );
        let content = fs::read_to_string(&path).expect("read fixture");
        let (_copyrights, holders, _authors) = detect_copyrights_from_text(&content);
    ...
    fn test_multilines_fixture_detects_split_copyright_by_holder() {
        let path =
            PathBuf::from("testdata/copyright-golden/copyrights/misco4/linux4/multilines.txt");
        let content = fs::read_to_string(&path).expect("read fixture");
    ...
    fn test_extract_from_tree_nodes_builds_hall_holder_tokens() {
        ...
        let (trailing_tokens, _skip) =
            collect_trailing_orphan_tokens(hall_node, &tree, hall_idx + 1);
...
Modifications for Debian Copyright (C) 1997-2007 James Troup.\n";
    ...
    fn test_detect_gnome_session_fixture_includes_queen_of_england() {
        let path = PathBuf::from(
            "testdata/copyright-golden/copyrights/gnome_session-gnome_session.copyright",
        );
 ...
 ****************************************************************************/\n";
    ...
    fn test_detect_composite_copy_copyrighted_by_with_trailing_copyright_clause() {
        let content = "FaCE is copyrighted by Object Computing, Inc., St. Louis Missouri, Copyright (C) 2002,\n";
        let numbered_lines: Vec<(usize, String)> = content
            .lines()
            .enumerate()
            .map(|(i, line)| (i + 1, line.to_string()))
    ...
    fn test_author_colon_multiline_keeps_emails() {
        ...
        extract_author_colon_blocks(input, &mut extracted);
        assert!(
    ...
    fn test_fixture_adler_inflate_not_copyrighted() {
        let content = std::fs::read_to_string(
            "testdata/copyright-golden/copyrights/adler_inflate_c-inflate_c.c",
        )
        .unwrap();
    ...
    fn test_refine_relay_tom_zanussi_line() {
        ...
        assert_eq!(
            refined,
            Some(
                "Copyright (c) 2002, 2003 - Tom Zanussi (zanussi@us.ibm.com), IBM Corp".to_string()
            )
        );
    }
 ...
 * Copyright (C) 2005 Hewlett-Packard Co.";
        ...
        let prepared_line = crate::copyright::prepare::prepare_text_line(
...
#   Originally by Gordon Matzigkeit <gord@gnu.ai.mit.edu>, 1996";
    ...
    fn test_debianized_by_fixture_extracts_author() {
        let content = std::fs::read_to_string(
            "testdata/copyright-golden/copyrights/misco4/debianized-by.txt",
        )
        .unwrap();
        let (_c, _h, a) = detect_copyrights_from_text(&content);
        let authors: Vec<&str> = a.iter().map(|a| a.author.as_str()).collect();
    ...
    fn test_final_agreement_fixture_extracts_created_by_author() {
        let content = std::fs::read_to_string(
            "testdata/copyright-golden/copyrights/misco4/final-agreement.txt",
        )
        .unwrap();
        let (_c, _h, a) = detect_copyrights_from_text(&content);
        let authors: Vec<&str> = a.iter().map(|a| a.author.as_str()).collect();
